...
def prediction_MRE(y_test,y_predict):
	...
	for k, y_real in enumerate(y_test):
		if y_real!=0:
			ape=np.abs((y_real-y_predict[k])/y_real)
		else:
			if y_predict[k]!=0:
				ape=np.abs((y_predict[k]-y_real)/y_predict[k]) # 即 1.0
			else:
				ape=0.0
		absolute_percent_error.append(ape)
	pre_MRE=np.mean(absolute_percent_error)
	return pre_MRE
...
# 注:MAPE是越大越好,所以最好不要和其他误差方法放在一起
def prediction_MAPE_array(y_test,y_predict):
	'''
	计算测试数据的预测准确度 a if a>b else b
	The best possible score is 1.0 and it can be negative.
	'''
	...
	for k, y_real in enumerate(y_test):
		if y_real!=0:
			...
			if y_predict[k]!=0:
				...
				ape=0.0
		absolute_percent_error.append(ape)
	MAPE=1-array(absolute_percent_error)
	return MAPE
...
def con_LSTM(y_series=None,zero_num=7): # 计算时间序列的差分d值
	...
	if sum(y_series[0:zero_num])==0:
		head_zeros=1
	else:
		head_zeros=0
	# 检查是否存在1次中间连续至少zero_num个为0的数据
	intermittent=0
	k=0
	...
	while True: 
		if k<=end_k:
			...
			break
	intermittent=intermittent-head_zeros
	if data_amount_check and intermittent>=1:
		return True
	else:
		return False
...
def con_ARIMA(y_series=None): # 计算时间序列的差分d值
	...
	if data_amount_check:
		# 检查是否可2阶内差分平稳
		df_obj=pd.DataFrame(y_series,columns=['quantity']) # 将Series(name=quantity)转化为DataFrame
		...
		while True:
			# 白噪声检验结果
			lbvalue,pvalue2=acorr_ljungbox(ts_diff,lags=1) # 注意返回的pvalue2是一个array
			rule_1=(pvalue2[0]<p_value_shreshold)
			# ADF检验,平稳性检验
			...
			if not (rule_1 and rule_2) and d < max_diff_time:
				...
				break
		return isStationarity
	return data_amount_check
...
def con_SARIMAX(y_series=None,season=7): 
	...
	if data_amount_check:
		# (1) 检查仅周期差分是否可平稳
		#进行周期差分
		ts_diff_p = y_series.diff(season) 
		ts_diff_p.dropna(inplace=True) #丢掉缺失值,If True, do operation inplace and return None.
		# 白噪声检验结果
		lbvalue,pvalue2=acorr_ljungbox(ts_diff_p,lags=1) 
		rule_1=(pvalue2[0]<p_value_shreshold)
		# ADF检验,平稳性检验
		adf,pvalue1,usedlag,nobs,critical_values,icbest= adfuller(ts_diff_p) 
		rule_2=(adf<critical_values['1%'] and adf<critical_values['5%'] and adf<critical_values['10%'] and pvalue1<0.01) 
		# 忽略白噪声检验 rule_1 and
		if rule_1 and rule_2:
			isStationarity=True
		...
		# (2) 检查1阶差分+周期差分是否可平稳
		# 1阶差分
		ts_diff_p_d = y_series.diff(1) # 不会改变y_series的值
		ts_diff_p_d.dropna(inplace=True) # 丢掉缺失值,If True, do operation inplace and return None.
		#进行周期差分
		ts_diff_p_d = y_series.diff(season) 
		ts_diff_p_d.dropna(inplace=True) #丢掉缺失值,If True, do operation inplace and return None.
		# 白噪声检验结果
		lbvalue,pvalue2=acorr_ljungbox(ts_diff_p_d,lags=1) 
		rule_1=(pvalue2[0]<p_value_shreshold)
		# ADF检验,平稳性检验
		adf,pvalue1,usedlag,nobs,critical_values,icbest= adfuller(ts_diff_p_d) 
		rule_2=(adf<critical_values['1%'] and adf<critical_values['5%'] and adf<critical_values['10%'] and pvalue1<0.01) 
		# 忽略白噪声检验 rule_1 and
		if rule_1 and rule_2:
			isStationarity=True
...
if __name__=='__main__':
	y_test=[1,0,2,0,4]
	y_predict=[0,1,0,0,3]
	MRE=prediction_MAPE(y_test,y_predict)
	print(MRE)

	# from get_data_from_db import *
	# # print(input_data_df.head(10))
	# obj_ids=input_data_df['id'].unique() 
	# # 循坏每个id
	# for index_no,id in enumerate(obj_ids): 
	# 	df_tmp=input_data_df[input_data_df['id']==id]
	# 	date_min=min(df_tmp['date'])
	# 	date_max=max(df_tmp['date'])
	# 	df_train_test=df_tmp[['date','quantity']] # 选取日期和销量列
