...
1. 时间序列模型
	...
	1.3 'ARMA':ts_ARMA
	1.4 'ARIMA':ts_ARIMA
	1.5 'SARIMAX':ts_SARIMAX
	1.6 'HWES':ts_holtwinters_ES
2. 时间序列_外来变量模型
	2.1 'ARIMAX':ts_ARIMAX
	2.2 'SARIMAX':ts_SARIMAX
3. 时间序列(合并)模型
	3.1 'SMA':ts_SMA
	3.2 'WMA':ts_WMA
	3.3 'SES':ts_SES
	...
	3.5 'ES3':ts_ES3
4. AUTO_ARIMA模型
	4.1 'AUTO_ARIMA':ts_AUTO_ARIMA # 效率太慢暂时不用
5. 变量定义
	5.1 timeseries_methods
	5.2 timeseries_methods
	5.3 timeseries_merge_methods
'''
...
class ts_ARIMA(object):
	...
	def fit(self): #训练模型
		...
		for p in [0,1,2,4,6,8,10]:
			for q in [0,1,2]:
				...
				try:
					# 注:不使用Kalman优化方式(method='css-mle' or method='mle'),因为运行效率太低,暂时放弃
					results_ARIMA = model.fit(disp=-1, method='css',trend='c')
				except:
					continue
	...
	def rolling_predict(self,forcast_period=2):
		...
		for _ in range(forcast_period):
			...
			model_tmp = rolling_model.fit(disp=-1, method='css',trend='c')
		print()
		return array(y_forcast)  # 返回array

# 1.5 'SARIMAX':ts_SARIMA
...
class ts_holtwinters_ES:
	...
	"""
	def __init__(self,ts_train,error_fun=None):
		self.ts_train  = ts_train.copy() # 时间序列
		self.error_fun = error_fun
		self.fittedModel = None

	def fit(self):
		self.fittedModel = sm.tsa.ExponentialSmoothing(self.ts_train,seasonal_periods=7, seasonal='add').fit()
		# self.fittedModel = sm.tsa.ExponentialSmoothing(self.ts_train).fit()
	def get_fittedvalues(self):   # 预测拟合值
		# 注指数平滑中,predict()方法是预测最近的1期
		self.fittedvalues=self.fittedModel.predict(start=0,end=len(self.ts_train)-1)
		return array(self.fittedvalues)  # array

	def predict(self,forcast_period=2):
		y_predict=self.fittedModel.forecast(forcast_period)
...
class ts_ARIMAX(object):
	...
	def __init__(self,ts_train,exog,error_fun=None):
		#统计量的P值小于显著性水平0.05，则可以以95%的置信水平拒绝原假设，
		#认为序列为非白噪声序列(否则，接受原假设，认为序列为纯随机序列)
		self.ts_train = ts_train
		self.exog=exog
	...
	def fit(self): #训练模型
		...
		for p in [0,1,2,4,6,8,10]:
			for q in [0,1,2]:
				model = ARIMA(self.ts_train,exog=self.exog,order=(p,self.d,q)) #使用差分后的数据进行Arima模型的训练
				try:
					# 注:不使用Kalman优化方式(method='css-mle'),因为运行效率太低,暂时放弃
	...
	def predict(self,forcast_period=2,exog=None):
		y_forcast = self.fittedModel.forecast(forcast_period,exog=exog)[0] 
		return y_forcast  # 返回array
...
class ts_SARIMAX(object):
	...
	def predict(self,forcast_period=2,exog=None):
		y_forcast = self.fittedModel.forecast(steps=forcast_period,exog=exog)  # 返回Series
		return y_forcast  # 返回array
...
class ts_SES: 
	...
	def fit(self):
		self.fittedModel = holtwinters.SimpleExpSmoothing(self.ts_train).fit(optimized=True)
...
# 测试外来变量
	...
	# exogenous_variables=np.array([[1.0,2,5,0,5,3,2,7,5,3,1,2,5,0,5,3,2,7,5,3],[1.0,3,5,0,3,3,2,7,3,3,1,2,3,0,5,3,3,7,3,3]])
	# exogenous_variables=np.transpose(exogenous_variables)
	# print(exogenous_variables)
	# model = ARIMA(ts, order=(3,1,1),exog=exogenous_variables) #使用差分后的数据进行Arima模型的训练
	# results_ARIMA = model.fit(disp=-1, method='css',trend ='nc')
	# y_fit=results_ARIMA.predict()
	# print('fitted values:')
	# print(y_fit)
	# haha=np.array([[1,2,4,3,5],[1,3,4,3,3]])
	# haha=np.transpose(haha)
	# pre_y=results_ARIMA.forecast(5,exog=haha)[0]
	# print(pre_y)
	# print(ts)
#####################################################
	# 测试指数平滑
	index=pd.date_range('5/1/2018',periods=20,freq='d')
	ts=pd.Series([1.0,2,3,4,3,6,3,7,3,5,1,2,3,4,3,6,3,7,3,5],index=index)
	# model = holtwinters.SimpleExpSmoothing(ts).fit(optimized=True)
